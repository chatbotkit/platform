...
SELECT 
  ...
  c.meta,
  c.createdAt,
  COUNT(CASE WHEN r.value > 0 THEN 1 END) as _upvoteCount,
  COUNT(CASE WHEN r.value < 0 THEN 1 END) as _downvoteCount,
  COUNT(r.id) as _countValue,
  'rating' as _countType
FROM Contact c
...
ORDER BY _countValue DESC, _upvoteCount DESC
LIMIT ?;
