...
// @note using local OpenAPI spec since TaxJar doesn't publish an official one
const DEFINITION = join(__dirname, 'taxjar.openapi.yaml')
...
describe('litmus tests', () => {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.taxjar.com/v2/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Authorization: 'Bearer YOUR_ACCESS_TOKEN',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['taxjar/api/call']
