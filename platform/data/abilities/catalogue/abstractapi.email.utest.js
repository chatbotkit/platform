...
import templates from './abstractapi.email'
...
const DEFINITION = join(__dirname, 'abstractapi.email.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://emailvalidation.abstractapi.com/v1?api_key=test&email=test@example.com'
    )
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://emailvalidation.abstractapi.com/v2/invalid-endpoint-that-does-not-exist'
    )
