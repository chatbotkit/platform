...
// @see https://api.apis.guru/v2/specs/giphy.com/1.0/openapi.yaml
const DEFINITION = 'https://api.apis.guru/v2/specs/giphy.com/1.0/openapi.json'
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://api.giphy.com/v1/gifs/search?api_key=test&q=cats',
      {
        headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.giphy.com/v1/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Accept: 'application/json',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['giphy/api/call']
