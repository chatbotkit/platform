...
// @note load the local OpenAPI spec file since Glimpse does not have a public spec
const DEFINITION = join(__dirname, 'glimpse.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://enterprise.meetglimpse.com/v1/interest?keyword=keto',
      {
        headers: {
          apikey: 'YOUR_API_KEY',
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://enterprise.meetglimpse.com/v1/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          apikey: 'YOUR_API_KEY',
          accept: 'application/json',
...
describe('templates', () => {
  ...
  const untestableTemplates = ['glimpse/api/call']
