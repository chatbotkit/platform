...
import templates from './abstractapi.phone'
...
const DEFINITION = join(__dirname, 'abstractapi.phone.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://phonevalidation.abstractapi.com/v1?api_key=test&phone=14152007986'
    )
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://phonevalidation.abstractapi.com/v2/invalid-endpoint-that-does-not-exist'
    )
