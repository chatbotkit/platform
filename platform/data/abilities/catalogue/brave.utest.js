...
beforeAll(async () => {
  // @note load the local OpenAPI spec file for Brave Search
  const { handlers } = await createOpenApiHandlers(
    join(__dirname, 'brave.openapi.yaml')
  )
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      ...
      {
        headers: {
          'X-Subscription-Token': 'YOUR_ACCESS_TOKEN',
        },
      }
    )
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.search.brave.com/res/v1/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          'X-Subscription-Token': 'YOUR_ACCESS_TOKEN',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['brave/api/call']
