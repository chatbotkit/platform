...
const DEFINITION =
  'https://raw.githubusercontent.com/ably/open-specs/refs/heads/main/definitions/platform-v1.yaml'
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://rest.ably.io/channels/test-channel/messages',
      {
        headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://rest.ably.io/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Authorization: 'Bearer YOUR_ACCESS_TOKEN',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['ably/api/call']
