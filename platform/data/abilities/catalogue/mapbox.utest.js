...
const DEFINITION = join(__dirname, 'mapbox.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid forward geocode endpoints', async () => {
    const response = await fetch(
      'https://api.mapbox.com/geocoding/v5/mapbox.places/San%20Francisco.json?access_token=test-token',
      {
        method: 'GET',
        headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.mapbox.com/invalid-endpoint-that-does-not-exist',
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
