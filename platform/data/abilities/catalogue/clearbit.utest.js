...
import templates from './clearbit'
...
const DEFINITION = join(__dirname, 'clearbit.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://person.clearbit.com/v2/people/find?email=test@example.com',
      {
        headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://person.clearbit.com/v2/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Authorization: 'Bearer YOUR_ACCESS_TOKEN',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['clearbit/api/call']
