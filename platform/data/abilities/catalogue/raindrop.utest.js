...
import templates from './raindrop'
...
// @note load the local OpenAPI spec file since Raindrop.io doesn't publish an official spec
const DEFINITION = join(__dirname, 'raindrop.openapi.yaml')
...
describe('litmus tests', () => {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.raindrop.io/rest/v1/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Authorization: 'Bearer YOUR_ACCESS_TOKEN',
        },
