...
const DEFINITION = 'https://api.apis.guru/v2/specs/asana.com/1.0/openapi.json'
...
describe('litmus tests', () => {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://app.asana.com/api/1.0/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Authorization: 'Bearer YOUR_ACCESS_TOKEN',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = [
    'asana/api/call', // generic API call template cannot be reliably tested with OpenAPI mocking
  ]
