...
const DEFINITION = join(__dirname, 'twitter.openapi.yaml')
...
describe('litmus tests', () => {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.x.com/2/invalid-endpoint-that-does-not-exist'
    )
...
describe('templates', () => {
  ...
  // @note twitter/sql/exec hits an internal auxiliary route (covered by the
  // route's own _sql.utest.js), twitter/api/call takes a free-form URL, and
  // packs install other abilities rather than making a request of their own.
  const untestableTemplates = [
    'twitter/sql/exec',
    'twitter/api/call',
    'pack/twitter',
    'pack/twitter[read-only]',
  ]
