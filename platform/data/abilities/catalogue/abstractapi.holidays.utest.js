...
import templates from './abstractapi.holidays'
...
const DEFINITION = join(__dirname, 'abstractapi.holidays.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://holidays.abstractapi.com/v1?api_key=test&country=US&year=2024'
    )
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://holidays.abstractapi.com/v2/invalid-endpoint-that-does-not-exist'
    )
