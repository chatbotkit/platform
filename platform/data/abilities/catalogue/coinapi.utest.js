...
// @note using local OpenAPI spec because the remote spec does not include the /v1/assets endpoint
const DEFINITION = join(__dirname, 'coinapi.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://rest.coinapi.io/v1/assets/BTC?apikey=test'
    )
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://rest.coinapi.io/v1/invalid-endpoint-that-does-not-exist'
    )
...
describe('templates', () => {
  ...
  const untestableTemplates = ['coinapi/api/call']
