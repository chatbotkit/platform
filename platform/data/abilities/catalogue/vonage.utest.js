...
const DEFINITION =
  'https://api.apis.guru/v2/specs/nexmo.com/sms/1.2.0/openapi.json'
...
beforeAll(async () => {
  const { handlers } = await createOpenApiHandlers(DEFINITION, {
    transformDefinition: (definition) => {
      // @note the Vonage SMS API uses query parameters for authentication
      // @note ensure the spec properly handles both POST and GET requests
      return definition
    },
  })
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://rest.nexmo.com/sms/json?api_key=test&api_secret=test',
      {
        method: 'POST',
        headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://rest.nexmo.com/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          'Content-Type': 'application/json',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['vonage/api/call']
