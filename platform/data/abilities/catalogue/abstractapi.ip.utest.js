...
import templates from './abstractapi.ip'
...
const DEFINITION = join(__dirname, 'abstractapi.ip.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://ipgeolocation.abstractapi.com/v1?api_key=test&ip_address=8.8.8.8'
    )
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://ipgeolocation.abstractapi.com/v2/invalid-endpoint-that-does-not-exist'
    )
