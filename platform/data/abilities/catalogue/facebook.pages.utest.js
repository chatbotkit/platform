...
import templates from './facebook.pages'
...
const DEFINITION = join(__dirname, 'facebook.pages.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://graph.facebook.com/v13.0/123456789/insights',
      {
        headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://graph.facebook.com/v13.0/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Authorization: 'Bearer TOKEN',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['facebook/api/call']
