...
import templates from './barcodelookup'
...
const DEFINITION = join(__dirname, 'barcodelookup.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://api.barcodelookup.com/v3/products?barcode=077341125112&key=test-key'
    )
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.barcodelookup.com/v3/invalid-endpoint-that-does-not-exist?key=test-key'
    )
