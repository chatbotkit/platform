...
// @note uplead does not have a public OpenAPI spec, using local barebone spec
const DEFINITION = join(__dirname, 'uplead.openapi.yaml')
...
describe('litmus tests', () => {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.uplead.com/v2/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Authorization: 'test-api-key',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['uplead/api/call']
