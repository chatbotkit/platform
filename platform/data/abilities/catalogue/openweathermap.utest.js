...
import templates from './openweathermap'
...
const DEFINITION = join(__dirname, 'openweathermap.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://api.openweathermap.org/data/2.5/weather?q=London&appid=test-key',
      {
        headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.openweathermap.org/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          'Content-Type': 'application/json',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['openweathermap/api/call']
