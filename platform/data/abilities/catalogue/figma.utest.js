  ...
  const { handlers } = await createOpenApiHandlers(DEFINITION, {
    transformDefinition: (definition) => {
      const getFileResponse =
        definition.components?.responses?.GetFileResponse?.content?.[
          'application/json'
        ]

      if (getFileResponse) {
        getFileResponse.example = {}
      }
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch('https://api.figma.com/v1/files/test-key', {
      headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.figma.com/v1/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          'X-Figma-Token': 'test-token',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['figma/api/call']
