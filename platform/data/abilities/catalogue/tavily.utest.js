...
beforeAll(async () => {
  ...
  const { handlers } = await createOpenApiHandlers(
    join(__dirname, 'tavily.openapi.yaml')
  )
...
describe('litmus tests', () => {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.tavily.com/invalid-endpoint-that-does-not-exist',
      {
        method: 'POST',
        headers: {
          Authorization: 'Bearer YOUR_ACCESS_TOKEN',
...
describe('templates', () => {
  ...
  const untestableTemplates = ['tavily/api/call']
