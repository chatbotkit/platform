...
import templates from './accuweather'
...
// @note using local barebone OpenAPI spec since AccuWeather does not provide a public spec
const DEFINITION = join(__dirname, 'accuweather.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'http://dataservice.accuweather.com/locations/v1/cities/search?apikey=test&q=Seattle',
      {
        headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'http://dataservice.accuweather.com/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Accept: 'application/json',
        },
