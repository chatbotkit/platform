...
const DEFINITION = join(__dirname, 'newsapi.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://newsapi.org/v2/everything?q=test&apiKey=test-key'
    )
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://newsapi.org/v2/invalid-endpoint-that-does-not-exist'
    )
