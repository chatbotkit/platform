...
beforeAll(async () => {
  ...
  const { handlers } = await createOpenApiHandlers(
    join(__dirname, 'ahrefs.openapi.yaml')
  )
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://api.ahrefs.com/v3/site-explorer/all-backlinks?target=example.com&select=url_from,anchor',
      {
        headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.ahrefs.com/v3/site-explorer/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Authorization: 'Bearer test-token',
        },
