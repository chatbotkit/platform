...
import templates from './dictionaryapi'
...
// @note load the local OpenAPI spec file for Free Dictionary API
const DEFINITION = join(__dirname, 'dictionaryapi.openapi.yaml')
...
describe('litmus tests', () => {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.dictionaryapi.dev/api/invalid-endpoint-that-does-not-exist'
    )
...
describe('templates', () => {
  ...
  // @note DictionaryAPI has a single template and is a public API (no auth required)
