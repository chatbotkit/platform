...
import templates from './abstractapi.vat'
...
const DEFINITION = join(__dirname, 'abstractapi.vat.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://vat.abstractapi.com/v1/validate?api_key=test&vat_number=SE556656688001'
    )
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://vat.abstractapi.com/v2/invalid-endpoint-that-does-not-exist'
    )
