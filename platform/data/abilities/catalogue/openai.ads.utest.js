...
import templates from './openai.ads'
...
const DEFINITION = join(__dirname, 'openai.ads.openapi.yaml')
...
describe('litmus tests', () => {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.ads.openai.com/v1/invalid-endpoint-that-does-not-exist'
    )
...
describe('templates', () => {
  ...
  // @note the generic API call template targets an AI-supplied URL which cannot
  // be reliably matched against the OpenAPI mocks
  const untestableTemplates = ['openai/ads/api/call']
