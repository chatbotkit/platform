...
import templates from './calendly'
...
// @note using the bundled/dereferenced version of the spec to avoid external $ref issues
const DEFINITION =
  'https://stoplight.io/api/v1/projects/calendly/api-docs/nodes/reference/calendly-api/openapi.yaml?fromExportButton=true&snapshotType=http_service&deref=optimizedBundle'
...
describe('litmus tests', () => {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.calendly.com/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          Authorization: 'Bearer YOUR_ACCESS_TOKEN',
        },
...
describe('templates', () => {
  ...
  const untestableTemplates = ['calendly/api/call']
