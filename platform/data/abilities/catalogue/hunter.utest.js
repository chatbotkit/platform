...
// @note hunter.io does not have a public OpenAPI spec, using local barebone spec
const DEFINITION = join(__dirname, 'hunter.openapi.yaml')
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://api.hunter.io/v2/email-finder?first_name=John&last_name=Doe&domain=example.com',
      {
        headers: {
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.hunter.io/v2/invalid-endpoint-that-does-not-exist',
      {
        headers: {
          'X-API-KEY': 'test-api-key',
        },
...
describe('templates', () => {
  ...
  // @note all hunter templates use createFetchTemplate and can be tested
  const untestableTemplates = ['hunter/api/call']
