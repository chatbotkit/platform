...
import templates from './geocodio'
...
const DEFINITION =
  'https://raw.githubusercontent.com/Geocodio/openapi-spec/master/geocodio-api.yml'
...
describe('litmus tests', () => {
  it('should accept valid endpoints', async () => {
    const response = await fetch(
      'https://api.geocod.io/v1.9/geocode?q=1600+Pennsylvania+Ave+NW,+Washington,+DC+20500&api_key=test_api_key'
    )
  ...
  it('should reject invalid endpoints', async () => {
    const response = await fetch(
      'https://api.geocod.io/v1.9/invalid-endpoint-that-does-not-exist?api_key=test_api_key'
    )
...
describe('templates', () => {
  ...
  const untestableTemplates = ['geocodio/api/call']
