...
export async function getServerSideProps(context) {
  const all = await prisma.hubBlueprintPage.findMany({
    take: MAX_FEED_ITEMS,
    ...
    toXML(
      {
        ...
        _content: {
          channel: [
            {
              title: 'ChatBotKit Latest Hub Blueprints',
            },
            {
              description: `The latest blueprints on the ChatBotKit Hub`,
            },
            {
              link: `${process.env.SITE_URL}/hub/blueprints/latest`,
            },
            {
              'atom:link': {
                _attrs: {
                  href: `${process.env.SITE_URL}/hub/blueprints/latest/rss.xml`,
                  rel: 'self',
            ...
            ...all.map((item) => ({
              item: [
                ...
                {
                  link: `${process.env.SITE_URL}/hub/blueprints/${
                    item.slug || item.id
                  }`,
                },
                ...getEnclosure(item),
