...
export async function handleCleanupEvent(payload) {
  ...
  // @todo perhaps delete all expired tokens not just refresh tokens, although
  // I believe that this case covers for both

  await prisma.oAuthApplicationToken.deleteMany({
    where: {
...
export async function sendEvent(event) {
  ...
  await queue(`/api/oauth/queue`, event)
}
