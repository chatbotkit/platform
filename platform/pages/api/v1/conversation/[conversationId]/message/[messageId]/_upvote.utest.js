...
import handler, { bodySchema } from './upvote'
...
describe('bodySchema (message-level upvote)', () => {
  ...
  it('should accept an optional reason string', () => {
    const result = bodySchema.validate({ value: 80, reason: 'Very helpful' })

    expect(result.error).toBeUndefined()
    expect(result.value.reason).toBe('Very helpful')
  })
...
describe('/api/v1/conversation/[conversationId]/message/[messageId]/upvote', () => {
  const mockSession = {
    user: { id: 'user_123' },
  }
  ...
  describe('basic functionality', () => {
    it('should upvote the message and return its id', async () => {
    ...
    it('should pass messageId to ratingLimitOK - key difference from conversation-level upvote', async () => {
      prisma.conversation.findUnique.mockResolvedValue(mockConversation)
      ratingLimitOK.mockResolvedValue(true)

      await handler(mockReq, mockSession, { value: 100 })

      // The message-level upvote MUST include messageId in the rate-limit check
      // (unlike the conversation-level upvote which omits it).
    ...
    it('should include reason in the rating when provided', async () => {
      ...
      expect(prisma.rating.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ reason: 'Clear answer' }),
        })
      )
    })
  })
  ...
  describe('authorization', () => {
    ...
    it('should not allow cross-user upvote: user B cannot upvote user A message', async () => {
      prisma.conversation.findUnique.mockResolvedValue({
        ...mockConversation,
        userId: 'user_A',
      })
