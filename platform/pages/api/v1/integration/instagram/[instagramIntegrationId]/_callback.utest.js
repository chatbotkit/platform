...
import handler from '@/pages/api/v1/integration/instagram/[instagramIntegrationId]/callback'
...
jest.mock(
  '@/pages/api/v1/integration/instagram/[instagramIntegrationId]/queue',
  () => ({
    sendEvent: jest.fn(),
  })
)
...
describe('Instagram callback API handler', () => {
  beforeEach(() => {
    mockReset(prisma)
    jest.clearAllMocks()
  })
  ...
  function makeRequest(
    payload,
    {
      instagramIntegrationId = 'int-123',
      method = 'POST',
      queryParams = {},
    } = {}
  ) {
    ...
    const url = `https://example.com/api/v1/integration/instagram/${instagramIntegrationId}/callback?${queryString}`
  ...
  describe('integration lookup', () => {
    it('returns notFound when integration does not exist', async () => {
      prisma.instagramIntegration.findUnique.mockResolvedValue(null)
    ...
    it('looks up integration by instagramIntegrationId from URL', async () => {
      ...
      const req = makeRequest({}, { instagramIntegrationId: 'int-456' })

      await handler(req)

      expect(prisma.instagramIntegration.findUnique).toHaveBeenCalledWith({
        where: { id: 'int-456' },
      })
    })
  })
  ...
  describe('message filtering', () => {
    ...
    it('skips unsupported message types', async () => {
      ...
      // Stickers and GIFs trigger this
  ...
  describe('realistic Meta webhook payloads', () => {
    it('processes story reply message', async () => {
      prisma.instagramIntegration.findUnique.mockResolvedValue({
        id: 'int-123',
        userId: 'user-1',
        verifyToken: 'test-token',
      ...
      // Actual Meta story reply format
      ...
      parseRequestJson.mockResolvedValue(storyReplyPayload)

      const req = makeRequest(storyReplyPayload)
      const res = await handler(req)

      expect(res.status).toBe(200)
      expect(sendEvent).toHaveBeenCalledWith('int-123', {
        type: 'interact',
        payload: storyReplyPayload.entry[0].messaging[0],
      })
    })

    it('processes story mention', async () => {
      prisma.instagramIntegration.findUnique.mockResolvedValue({
        id: 'int-123',
        userId: 'user-1',
        verifyToken: 'test-token',
      ...
      parseRequestJson.mockResolvedValue(storyMentionPayload)

      const req = makeRequest(storyMentionPayload)
      const res = await handler(req)

      expect(res.status).toBe(200)
      expect(sendEvent).toHaveBeenCalledWith('int-123', {
        type: 'interact',
        payload: storyMentionPayload.entry[0].messaging[0],
      })
    })

    it('processes reel share', async () => {
      prisma.instagramIntegration.findUnique.mockResolvedValue({
        id: 'int-123',
        userId: 'user-1',
        verifyToken: 'test-token',
      ...
      parseRequestJson.mockResolvedValue(reelSharePayload)

      const req = makeRequest(reelSharePayload)
      const res = await handler(req)

      expect(res.status).toBe(200)
      expect(sendEvent).toHaveBeenCalledWith('int-123', {
        type: 'interact',
        payload: reelSharePayload.entry[0].messaging[0],
      })
    })
    ...
    it('processes message from Instagram Shop product page', async () => {
      prisma.instagramIntegration.findUnique.mockResolvedValue({
        id: 'int-123',
        userId: 'user-1',
        verifyToken: 'test-token',
      ...
      parseRequestJson.mockResolvedValue(productReferralPayload)

      const req = makeRequest(productReferralPayload)
      const res = await handler(req)

      expect(res.status).toBe(200)
      expect(sendEvent).toHaveBeenCalledWith('int-123', {
        type: 'interact',
        payload: productReferralPayload.entry[0].messaging[0],
      })
    })
    ...
    it('processes ephemeral (disappearing) media', async () => {
      prisma.instagramIntegration.findUnique.mockResolvedValue({
        id: 'int-123',
        userId: 'user-1',
        verifyToken: 'test-token',
            ...
            messaging: [
              {
                ...
                message: {
                  ...
                  attachments: [
                    {
                      type: 'ephemeral',
                      // No URL included for ephemeral media per Meta docs
                    },
                  ],
                },
              },
      ...
      parseRequestJson.mockResolvedValue(ephemeralPayload)

      const req = makeRequest(ephemeralPayload)
      const res = await handler(req)

      expect(res.status).toBe(200)
      expect(sendEvent).toHaveBeenCalledWith('int-123', {
        type: 'interact',
        payload: ephemeralPayload.entry[0].messaging[0],
      })
    })
    ...
    it('processes Click To Direct (CTD) ad message', async () => {
      prisma.instagramIntegration.findUnique.mockResolvedValue({
        id: 'int-123',
        userId: 'user-1',
        verifyToken: 'test-token',
      ...
      parseRequestJson.mockResolvedValue(ctdAdPayload)

      const req = makeRequest(ctdAdPayload)
      const res = await handler(req)

      expect(res.status).toBe(200)
      expect(sendEvent).toHaveBeenCalledWith('int-123', {
        type: 'interact',
        payload: ctdAdPayload.entry[0].messaging[0],
      })
    })
  })
  ...
  describe('signature verification', () => {
    ...
    function signedRequest(body, signature) {
      return new Request(
        'https://example.com/api/v1/integration/instagram/int-123/callback?instagramIntegrationId=int-123',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
