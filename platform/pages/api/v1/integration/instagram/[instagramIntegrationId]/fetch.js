 ...
 *   get:
 *     operationId: fetchInstagramIntegration
 *     summary: Fetch an Instagram integration
 *     tags:
 ...
 *         required: true
 *         schema:
 *           description: The ID of the Instagram integration to retrieve
 *           type: string
 *     responses:
 *       200:
...
export default withGet(
  withSession(async function (req, session) {
    ...
    delete (/** @type {any} */ (instagramIntegration).userId)

    if (instagramIntegration.accessToken) {
      /** @type {any} */ instagramIntegration.accessToken = '********'
    }

    if (instagramIntegration.appSecret) {
      /** @type {any} */ (instagramIntegration).appSecret = '********'
    }

    return ok(makeJsonSafe(instagramIntegration))
  })
)
 ...
 *
 * Retrieve complete configuration details for a specific Instagram integration,
 ...
 * Instagram integration. The response includes all configuration options and
 * their current values, enabling you to display complete integration settings
 * or use the data to populate update forms:
 *
 * ```http
 * GET /api/v1/integration/instagram/{instagramIntegrationId}/fetch
 ...
 * status indicators.
 *
 * **Setup Verification**: Verify that all required fields (access token)
 * have been configured before enabling the integration.
 *
