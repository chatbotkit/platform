...
describe('POST recall session screenshare stop', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  })

  it('stops screenshare for a valid recall session', async () => {
    ...
    const response = await stopHandler(
      makeRequest(
        '/api/v1/integration/recall/recall-1/session/session-1/screenshare/stop?recallIntegrationId=recall-1&sessionId=session-1'
      )
    )
