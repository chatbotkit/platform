  ...
  withSession(
    withSchema(bodySchema, async function (req, session, body) {
      ...
      const { flagged, categories } = await detectContentAbuse(
        joinTrimmedNotEmpty(
          [name || skillset.name, description || skillset.description],
          '\n\n'
        )
      )
      ...
      const { id } = await prisma.hubSkillsetPage.upsert({
        where: {
        ...
        create: {
          ...
          name: name || skillset.name,
          description: description || skillset.description,
