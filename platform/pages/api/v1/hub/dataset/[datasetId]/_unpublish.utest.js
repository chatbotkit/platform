...
describe('/api/v1/hub/dataset/[datasetId]/unpublish', () => {
  ...
  it('returns 200 and deleted hub page id for owner', async () => {
    ...
    prisma.hubDatasetPage.delete.mockResolvedValue({ id: 'hub_page_1' })
    ...
    expect(prisma.hubDatasetPage.delete).toHaveBeenCalledWith({
      where: { datasetId: 'ds_1' },
      select: { id: true },
    })
    expect(result).toEqual({
      status: 200,
      body: { id: 'hub_page_1', datasetId: 'ds_1' },
    })
  })
  ...
  it('returns 404 when dataset is missing', async () => {
    ...
    expect(prisma.hubDatasetPage.delete).not.toHaveBeenCalled()
  })
  ...
  it('propagates prisma deletion errors', async () => {
    ...
    prisma.hubDatasetPage.delete.mockRejectedValue(new Error('delete failed'))
