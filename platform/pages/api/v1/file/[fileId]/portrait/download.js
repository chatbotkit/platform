...
import { createPortrait, isSupportedImageType } from '@/lib/image.transform'
...
export default withGet(async function (req) {
  ...
  if (isSupportedImageType(contentType)) {
    try {
      ...
      const { buffer: portraitBuffer, mimeType } = await createPortrait(
        imageData,
        { contentType }
      )

      return send(portraitBuffer, {
...
// @todo add comprehensive documentation about portrait transformations including caching, version control, and advanced features
