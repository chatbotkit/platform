import { searchHandler } from '@chatbotkit-dev/auxiliary-notion'
...
export default authenticatedHandler(
  ...
  async function (_session, parameters, headers) {
    debug(`notion/search`, { parameters, headers }).log(
      'auxiliary.skillset.ability.notion.search'
    )

    const { query, startCursor, pageSize, simplifiedProperties } = parameters
    ...
    return await searchHandler({
      token: token,
      query: query && isTrimmedNonEmptyString(query) ? query : undefined,
      startCursor:
