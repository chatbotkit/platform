...
import faq from '@/content/faqs/platform-integrations-messenger.yaml'
    ...
    endpoints: [
      {
        ...
        description:
          'Copy the callback endpoint above and set it as the Webhook Callback URL in your Messenger configuration.',
        required: true,
        copyMessage: 'Messenger callback endpoint copied to clipboard',
      },
    ],
    ...
    secrets: [
      {
        ...
        required: true,
        copyMessage: 'Messenger verify token copied to clipboard',
      },
    ],
...
export function Form({ integration, installDetails }) {
  ...
  async function handleOnSubmit(event) {
    ...
    if (integration.id) {
      const { error } = await fetch(
        `/api/v1/integration/messenger/${integration.id}/update`,
        {
          data: {
            ...data,
          },

          successMessage: 'Messenger integration settings updated.',
        }
      )
      ...
      const {
        data: { id: messengerIntegrationId },
      } = await fetch(`/api/v1/integration/messenger/create`, {
        data: scopeCreateData(data),
      })

      Object.assign(integration, data)

      if (messengerIntegrationId) {
        router.push(`/integrations/messenger/${messengerIntegrationId}`)
      }
    }
  }
  ...
  async function handleIntegrationHelp(event) {
    ...
    if (
      !(await confirm(
        'Please contact support if you need help with your Messenger Business integration.'
      ))
    ) {
      return
    }
  ...
  async function handleDelete(event) {
    ...
    const { error } = await fetch(
      `/api/v1/integration/messenger/${integration.id}/delete`,
      {
        data: {},

        successMessage: 'Messenger integration deleted...',
      }
    )
  ...
  async function handleSetup(event) {
    ...
    await fetch(`/api/v1/integration/messenger/${integration.id}/setup`, {
      data: {},

      successMessage: 'Messenger setup completed.',
    })
  }
    ...
    <>
      ...
      <form className="divided-area" onSubmit={handleOnSubmit}>
        <div className="divided-area">
          ...
          <div>
            <Headline title="Messenger Integration Configuration">
          ...
          {integration.id ? (
            <>
              ...
              <div>
                <Headline title="Messenger Application Configuration">
                  This information is used to configure the messenger
                  integration.
                </Headline>
                <div className="mt-6 space-y-6">
                  {/* access token */}
                  <div>
                    ...
                    <p className="input-description">
                      The access token for this Messenger application. You need
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="input-description">
                        Collect contact details such as name.
                      </p>
                    </div>
                    {/* sessionDuration */}
                    <div>
          ...
          <div className="action-area">
            ...
            {integration.id ? (
              <IntegrationInstallButton
                title="Messenger Install Instructions"
...
export function Initiate({ integration }) {
  const { loading, fetch } = useFetch({
    loadingMessage: 'Initiating Messenger conversation...',
    failureMessage: true,
    successMessage: 'Messenger conversation initiated.',
  })
  ...
  async function handleInitiate(event) {
    ...
    await fetch(`/api/v1/integration/messenger/${integration.id}/initiate`, {
      data,
    })
  }
  ...
  return (
    <form className="space-y-6" onSubmit={handleInitiate}>
      <div className="grid gap-6 sm:grid-cols-2">
        <div>
          <label className="default-label" htmlFor="pageId">
            Page ID
          </label>
          <div className="mt-1">
            <input
          ...
          <p className="input-description">
            Use the Facebook Page ID sending the message.
          </p>
        </div>
        <div>
          ...
          <p className="input-description">
            Use a Messenger recipient PSID from a prior interaction.
          </p>
        </div>
      </div>
      <div>
        ...
        <div className="mt-1">
          <AutoTextarea
            ...
            className="default-input w-full sm:text-sm"
            name="text"
            placeholder="Write the free-form Messenger message..."
            required
          />
        </div>
        <p className="input-description">
          This sends a free-form Messenger message while Meta allows messaging
          this recipient.
        </p>
      </div>
      <div className="action-area">
  ...
  return (
    <>
      {/* <div className="main-page last">
        ...
        >
          <p>
            With this integration, you can create your own Messenger
            conversational AI chatbot. Detailed instructions on how to set up
            this integration can be found at{' '}
            <DocsLink className="default-link" slug="messenger">
              ChatBotKit Messenger Integration
            </DocsLink>{' '}
            docs.
          </p>
        </NavHeader>
      ...
      <PageSections className="pt-12">
        ...
        {integration.id ? (
          <section data-page-section-title="Webhook">
            <div className="main-page">
              <Headline title="Messenger Webhook Configuration">
        ...
        {integration.id ? (
          <section data-page-section-title="Initiate">
            <div className="main-page">
              <Headline title="Messenger Initiate">
                Start a Messenger conversation from this integration.
              </Headline>
              <Expando
                titleClassName="default-link text-sm"
                title="Show Initiate"
        ...
        {integration.id ? (
          <section data-page-section-title="Events">
            <div className="main-page">
              <Headline title="Messenger Integration Events">
                Keep tabs on the progress of your Messenger integration&apos;s
                events.
              </Headline>
              <EventLog
                eventTypes={VISIBLE_EVENT_TYPES}
                autoLoad={true}
                contextFilters={{ messengerIntegrationId: integration.id }}
                filter={false}
              />
            </div>
          </section>
...
Index.getLayout = function (children, { integration }) {
  return (
    <Dashboard
      breadcrumbs={['Messenger', 'Integrations', 'ChatBotKit']}
...
export async function getServerSideProps(context) {
  ...
  if (context.query.messengerIntegrationId === 'new') {
    return {
      props: makeJsonSafe({
        integration: {
          // url parameters
  ...
  const integration = await prisma.messengerIntegration.findUnique({
    where: {
      id: context.query.messengerIntegrationId,
    },
  ...
  return {
    props: makeJsonSafe({
      ...
      callbackEndpoint: getExternalAPIHostURL(
        `/v1/integration/messenger/${integration.id}/callback`
      ),
    }),
  }
}
