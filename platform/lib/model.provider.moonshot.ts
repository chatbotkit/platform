...
/**
 * Gets the Moonshot AI API key from model store or environment.
 *
 * @note Moonshot is BYOK-only: there is no platform env key, so `envKey` is left
...
export function getMoonshotAPIKey(): string {
  ...
  return resolveProviderCredential({
    label: 'Moonshot',
    storeKey: store.moonshotKey,
    storeUrl: store.moonshotUrl,
    envKey: undefined,
  })
}
