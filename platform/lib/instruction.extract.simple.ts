...
/**
 * Extracts parameter fields from a simple instruction and returns them as
 ...
 */
export function extractSimpleInstructionFields(
  instruction: string
): FieldSchema[] {
  debug(`extracting fields from simple instruction`, { instruction }).log(
    'instruction.simple.extractSimpleInstructionFields'
  )
  ...
  const fields = rawFields
    ...
    .map((field: ExtractedField) => {
      ...
      // @note set min and max for number type if specified in operand

      const minValue = getFieldValueMin(field)
      const maxValue = getFieldValueMax(field)
      ...
      // @note set local flag for fields prefixed with "local."
  ...
  debug(`extracted fields from simple instruction`, { fields }).log(
    'instruction.simple.extractSimpleInstructionFields'
  )
...
/**
 * Substitutes field values into a simple instruction string.
 *
 ...
 */
export function substituteSimpleInstructionFields(
  instruction: string,
  fieldValues: Record<string, unknown>
): string {
  debug(`substituting fields in simple instruction`, {
    instruction,
    fieldValues,
  }).log('instruction.simple.substituteSimpleInstructionFields')
  ...
  debug(`substituted fields in simple instruction`, { result }).log(
    'instruction.simple.substituteSimpleInstructionFields'
  )
