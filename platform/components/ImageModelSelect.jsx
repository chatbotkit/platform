...
import { buildImageModel, parseImageModel } from '@/lib/model.utils'
...
const imageModelCreditConsumptionLabels = {
  free: 'free',
  low: 'low credit',
...
const imageModelCreditConsumptionThresholds = {
  low: 2500,
  medium: 5000,
  high: 10000,
}

function getImageModelCreditConsumptionTier(modelConfig) {
  ...
  if (tokenRatio < imageModelCreditConsumptionThresholds.low) {
    return 'low'
  }

  if (tokenRatio < imageModelCreditConsumptionThresholds.medium) {
    return 'medium'
  }

  if (tokenRatio < imageModelCreditConsumptionThresholds.high) {
    return 'high'
  }
...
export function getImageModelSortPriority(modelName, modelConfig) {
  if (modelName === defaultImageModel) {
    return 0
  }
...
export function compareVisibleImageModels([aName, aConfig], [bName, bConfig]) {
  const priorityDifference =
    getImageModelSortPriority(aName, aConfig) -
    getImageModelSortPriority(bName, bConfig)
...
function getImageModelTokenRatio(modelConfig) {
  const tokenRatio = modelConfig?.pricing?.tokenRatio

  return typeof tokenRatio === 'number' ? tokenRatio : Infinity
}

function compareVisibleImageModelsBySort(sort) {
  return (a, b) => {
    ...
    switch (sort) {
      case 'cost': {
        const ratioDifference =
          getImageModelTokenRatio(aConfig) - getImageModelTokenRatio(bConfig)
      ...
      default:
        return compareVisibleImageModels(a, b)
    }
  }
}
...
function SelectPopup({ currentModel, onSelect, availableModels }) {
  ...
  const filteredModels = useMemo(() => {
    let models = Object.entries(visibleImageModels)
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm">
        Select an image model from the list below. You can configure additional
        parameters after selecting a model.
      </p>
      <div className="default-input flex items-center gap-2 !p-1.5">
        ...
        {imageModelSortOptions.map(({ id, label }) => (
          <button
            key={id}
            className={clsx(
      ...
      <div className="max-h-[500px] h-screen flex flex-col overflow-auto">
        <List>
          {filteredModels
            .sort(compareVisibleImageModelsBySort(sort))
            .map(([modelName, modelConfig]) => {
              const creditConsumptionTier =
                getImageModelCreditConsumptionTier(modelConfig)
                ...
                >
                  {creditConsumptionTier ? (
                    <span className="relative inline-flex group/tooltip cursor-help">
                      <span className="tag">
                        {
                          imageModelCreditConsumptionLabels[
                            creditConsumptionTier
                          ]
                        }
                      </span>
...
export function ConfigPopup({ name, config }) {
  return (
    <div className="space-y-6">
      {/* description */}
      {imageModels[name] ? (
        <p className="text-sm">{imageModels[name].description}</p>
      ) : null}
      {/* @todo add the ability to specify the image size */}
      {/* region */}
      <div>
        ...
        <div className="mt-1">
          <select
            ...
            name="region"
            defaultValue={either(config.region, imageModels[name].region)}
          >
            {[/* '', */ ...imageModels[name].availableRegions].map((region) => {
              return (
        ...
        <p className="input-description">
          ...
          is <strong>{imageModels[name].region}</strong>.
        </p>
      </div>
    </div>
  )
...
export default function ImageModelSelect({
  ...
  const availableDefaultModel = useAvailableDefaultModel('image')
  ...
  useEffect(() => {
    ...
    try {
      model = buildImageModel(modelName, modelConfig)
    } catch (e) {
      toast.error(e.message)
  ...
  const displayName = modelName || availableDefaultModel || defaultImageModel
