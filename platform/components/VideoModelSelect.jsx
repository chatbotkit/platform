...
import { buildVideoModel, parseVideoModel } from '@/lib/model.utils'
...
const videoModelCreditConsumptionLabels = {
  free: 'free',
  low: 'low credit',
...
const videoModelCreditConsumptionThresholds = {
  low: 5000,
  medium: 10000,
  high: 15000,
}

function getVideoModelCreditConsumptionTier(modelConfig) {
  ...
  if (tokenRatio < videoModelCreditConsumptionThresholds.low) {
    return 'low'
  }

  if (tokenRatio < videoModelCreditConsumptionThresholds.medium) {
    return 'medium'
  }

  if (tokenRatio < videoModelCreditConsumptionThresholds.high) {
    return 'high'
  }
...
export function getVideoModelSortPriority(modelName, modelConfig) {
  if (modelName === defaultVideoModel) {
    return 0
  }
...
export function compareVisibleVideoModels([aName, aConfig], [bName, bConfig]) {
  const priorityDifference =
    getVideoModelSortPriority(aName, aConfig) -
    getVideoModelSortPriority(bName, bConfig)
...
function getVideoModelTokenRatio(modelConfig) {
  const tokenRatio = modelConfig?.pricing?.tokenRatio

  return typeof tokenRatio === 'number' ? tokenRatio : Infinity
}

function compareVisibleVideoModelsBySort(sort) {
  return (a, b) => {
    ...
    switch (sort) {
      case 'cost': {
        const ratioDifference =
          getVideoModelTokenRatio(aConfig) - getVideoModelTokenRatio(bConfig)
      ...
      default:
        return compareVisibleVideoModels(a, b)
    }
  }
}
...
function SelectPopup({ currentModel, onSelect, availableModels }) {
  ...
  const filteredModels = useMemo(() => {
    let models = Object.entries(visibleVideoModels)
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm">
        Select a video model from the list below. You can configure additional
      ...
      <div className="default-input flex items-center gap-2 !p-1.5">
        ...
        {videoModelSortOptions.map(({ id, label }) => (
          <button
            key={id}
            className={clsx(
      ...
      <div className="max-h-[500px] h-screen flex flex-col overflow-auto">
        <List>
          {filteredModels
            .sort(compareVisibleVideoModelsBySort(sort))
            .map(([modelName, modelConfig]) => {
              const creditConsumptionTier =
                getVideoModelCreditConsumptionTier(modelConfig)
                ...
                >
                  {creditConsumptionTier ? (
                    <span className="relative inline-flex group/tooltip cursor-help">
                      <span className="tag">
                        {
                          videoModelCreditConsumptionLabels[
                            creditConsumptionTier
                          ]
                        }
                      </span>
...
export function ConfigPopup({ name, config }) {
  return (
    <div className="space-y-6">
      {videoModels[name] ? (
        <p className="text-sm">{videoModels[name].description}</p>
      ) : null}
      <div>
        ...
        <div className="mt-1">
          <select
            ...
            name="region"
            defaultValue={either(config.region, videoModels[name].region)}
          >
            {[...videoModels[name].availableRegions].map((region) => {
              return (
        ...
        <p className="input-description">
          ...
          is <strong>{videoModels[name].region}</strong>.
        </p>
      </div>
    </div>
  )
...
export default function VideoModelSelect({
  ...
  const availableDefaultModel = useAvailableDefaultModel('video')
  ...
  useEffect(() => {
    ...
    try {
      model = buildVideoModel(modelName, modelConfig)
    } catch (e) {
      toast.error(e.message)
  ...
  const displayName = modelName || availableDefaultModel || defaultVideoModel
